...
xdp_input_capture_pointer_barrier_class_init (XdpInputCapturePointerBarrierClass *klass)
{
  ...
  /**
   * XdpInputCapturePointerBarrier::active:
   * @barrier: the [class@InputCapturePointerBarrier] object
   * @is_active: true if active, false if failed
   *
   * Emitted when a pointer barrier was applied or failed to apply to the
   * current session. If the barrier's "active" state is %FALSE, the caller
   * can clean up the barrier.
   */
  signals[SIGNAL_ACTIVE] =
    g_signal_new("active",
...
void
_xdp_input_capture_pointer_barrier_set_active (XdpInputCapturePointerBarrier *barrier, gboolean active)
{
  ...
  g_object_notify_by_pspec (G_OBJECT (barrier), properties[PROP_ACTIVE]);
}
